...
import LaunchIcon from "@/components/icons/links/LaunchIcon";

import { blockScanDomain } from "@/utils/tizz";
...
export default function TradeHistoryModal({
  ...
  return (
    ...
    >
      <ModalContent>
        <ModalBody>
          {tradeHistory.tx && tradeHistory.tx.trim().length > 0 ? (
            <a
              href={`${blockScanDomain}/tx/${tradeHistory.tx}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              <LaunchIcon size={24} className="-mt-4 text-stone-200" filled />
            </a>
          ) : null}
