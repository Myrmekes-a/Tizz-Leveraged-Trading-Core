...
import { collateralPairIds, tizzContractAddresses } from "@/utils/tizz";
...
import { useDelayedUpdate } from "@/tizz-trade-hooks/useDelayedUpdate";
...
export default function OpenTradeModal({
  ...
  const { data: lpData, status: lpStatus } = useReadContract({
    address: tizzContractAddresses[collateralType].tizzFundingRate as Address,
  ...
  const cachedLP = useDelayedUpdate(lpData, lpStatus);
  ...
  const handleConfirm = async () => {
    ...
    writeContract(
      {
        abi: TizzTradingAbi,
        address: tizzContractAddresses[collateralType].tizzTrading as Address,
  ...
  const tradeInfos = [
    ...
    },
    {
      label: t("liq-price"),
      value:
        cachedLP !== undefined ? getPriceStr(Number(cachedLP) / 1e10) : "-",
    },
    {
      label: t("stop-loss"),
