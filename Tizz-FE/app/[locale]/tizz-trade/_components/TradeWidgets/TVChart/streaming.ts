...
import { BackendStatus, Chart } from "@/types/index";
...
import { BACKEND_STATUS_KEY } from "@/tizz-trade-hooks/useBackendStatus";
...
if (socket) {
  ...
  socket.on("backend-status", (status) => {
    if (status) {
      queryClient.setQueryData(
        [BACKEND_STATUS_KEY],
        (oldData: BackendStatus) => {
          return {
            ...oldData,
