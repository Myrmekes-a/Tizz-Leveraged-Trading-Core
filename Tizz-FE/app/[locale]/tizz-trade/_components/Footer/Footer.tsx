...
import { useBackendStatus } from "@/tizz-trade-hooks/useBackendStatus";
...
export default function Footer() {
  ...
  const hasServerError = data
    ? Object.values(data).find((item) => item === false)
    : false;
  ...
  return (
    <footer className="fixed bottom-0 flex w-full items-center justify-between border-t border-t-stroke bg-tizz-background">
      <div className="flex items-center gap-2 p-1">
        <TextButton
          className={twMerge(
            "px-2 py-[2px] text-[11px] opacity-50",
            hasServerError ? "text-red-400" : "text-stone-200",
          )}
        >
          <span
            className={twMerge(
              "h-3 w-3 rounded-md bg-emerald-400",
              hasServerError ? "bg-red-400" : "bg-emerald-400",
            )}
          />
          {hasServerError ? "Server Error" : "Operational"}
        </TextButton>
