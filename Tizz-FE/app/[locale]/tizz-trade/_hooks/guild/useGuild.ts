...
import {
  sendGuildInvitationRequestByAddress,
  InvitationRequestDto,
} from "@/tizz-trade-actions/client/guild/guild/sendGuildInvitationRequestByAddress";
...
import {
  ...
  GuildMember,
  leaveGuild,
} from "@/tizz-trade-actions/client/guild/guild/leaveGuild";
...
export function useGuild() {
  ...
  >({
    ...
    onError: (error) => {
      enqueueSnackbar(
        error.message?.join("\n") || error.error || "Failed at kicking user",
        {
          persist: false,
          variant: "error",
        },
  ...
  const sendInvitationMutation = useMutation<
    ...
    InvitationRequestDto
  >({
    mutationFn: sendGuildInvitationRequestByAddress,
  });
  ...
  >({
    ...
    onSuccess: (leavedMember: GuildMember) => {
      queryClient.setQueryData<IGuildWithAggregation>(
        ["getGuildById", leavedMember.guild_id],
        (data) => {
          if (!data) {
            return data;
          }
          ...
          return {
            ...data,
            guildMembers: data.guildMembers.filter(
              (item) => item.user_id !== leavedMember.user_id,
            ),
          };
        },
      );
    ...
    onError: (error) => {
      enqueueSnackbar(
        error.message?.join("\n") || error.error || "Failed at leaving a guild",
        {
          persist: false,
          variant: "error",
        },
