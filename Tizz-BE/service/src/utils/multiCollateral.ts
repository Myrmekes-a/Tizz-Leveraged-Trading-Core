...
export const initStack = async (w3, stackConfig) => {
  try {
    ...
    const token = new w3.eth.Contract(ABIS.TOKEN, stackConfig.token);
    ...
    const fundingFees = new w3.eth.Contract(ABIS.FUNDING_RATE, stackConfig.fundingRate);
    // We replaced borrowing fee with funding fee.
    const borrowingFees = fundingFees;
    
    const priceUsd = (await aggregator.methods.getCollateralPriceUsd().call()) + '';
    const price = (priceUsd as unknown as number) / 1e10; // 1e10 USD from feed
    ...
    console.error('error while init stack: ', error)
