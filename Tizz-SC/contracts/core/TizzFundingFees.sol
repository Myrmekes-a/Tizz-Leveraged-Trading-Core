 ...
 */
contract TizzFundingFees is Initializable, ITizzFundingFees {
    // Constants
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => InitialAccFees)))
    ...
    uint256 public FUNDING_RATE_PRECISION;
    ...
    modifier onlyManager() {
        if (
            !multiCollatDiamond.hasRole(
                msg.sender,
                IAddressStoreUtils.Role.MANAGER
            )
        ) {
            revert WrongAccess();
        }
    ...
    modifier onlyCallbacks() {
        if (
            !multiCollatDiamond.hasRole(
                msg.sender,
                IAddressStoreUtils.Role.CALLBACKS
            )
        ) {
            revert WrongAccess();
        }
    ...
    modifier onlyFeeUpdater() {
        if (
            !multiCollatDiamond.hasRole(
                msg.sender,
                IAddressStoreUtils.Role.FEE_UPDATER
            )
        ) {
            revert WrongAccess();
        }
    ...
    function updateBaseRate(uint256 _baseRate) external onlyManager {
        baseRate = _baseRate;
    }

    // Update funding fees
    /// @notice This function is calculating and update funding fees by a pair.
    /// @dev In the funding rate, if market is bullish, long positions pay to traders on short side. (positive)
    ///                           if market is bearish, short positions pay to traders on long side. (negative)
    /// @dev market_price > perps_price => bullish
    ///      market_price < perps_price => bearish
    /// @dev funding rate = abs((perps price - index price) / index price) + base rate;
    ///      fee = funding rate * position size
    ///      perps price = average of open prices, index price = current market price
    function syncFundingFee(
        uint256 pairIndex,
        bytes calldata proofbytes
    ) external onlyFeeUpdater {
        _syncFundingFee(pairIndex, proofbytes);
    }

    function syncFundingFees(
        uint256[] calldata pairIndices,
        bytes calldata proofbytes
    ) external onlyFeeUpdater {
        for (uint256 i = 0; i < pairIndices.length; i++) {
            _syncFundingFee(pairIndices[i], proofbytes);
        }
    }

    function _syncFundingFee(
        uint256 pairIndex,
        bytes calldata proofbytes
    ) internal {
        uint256 pairId = (multiCollatDiamond.pairs(pairIndex)).pairId;
        uint256 marketPrice = priceAggregator.GetPairPrice(proofbytes, pairId);
        uint256 perpsPrice = storageT.getPersPrice(pairIndex);
        if (perpsPrice == 0) {
            return;
        }

        bool isBullish = marketPrice > perpsPrice;
        uint256 priceDelta = isBullish
            ? marketPrice - perpsPrice
            : perpsPrice - marketPrice;
        uint256 fundingRate = (priceDelta * FUNDING_RATE_PRECISION) /
            marketPrice +
            baseRate;

        (uint256 oiLong, uint256 oiShort) = getPairOpenInterestBaseAsset(
            pairIndex
        );
        uint256 positionSize = isBullish ? oiLong : oiShort;
        Pair storage p = pairs[pairIndex];
        p.fundingFees = (fundingRate * positionSize) / FUNDING_RATE_PRECISION;
        p.fundingRate = fundingRate;
        p.isBullish = isBullish;
    }
    ...
    {
        return FundingFeesUtils.getPendingAccFees(input, isBullish);
    }

    function getFundingFees(
        uint256 pairIndex
    ) external view returns (uint256 fundingFees, uint256 fundingRate) {
        return (pairs[pairIndex].fundingFees, pairs[pairIndex].fundingRate);
    }
    ...
    function handleTradeAction(
        ...
        uint256 pairIndex,
        uint256 index,
        uint256 positionSizeBaseAsset, // 1e18 | 1e6 (collateral * leverage)
        bool open,
        bool long
        ...
        _setGroupOi(groupIndex, long, open, positionSizeBaseAsset);
    ...
    function getTradeFundingFee(
        ...
        uint256 fundingRate = pairs[input.pairIndex].fundingRate;
        uint256 collateralFundingFee = 0;
        if (input.long == pairs[input.pairIndex].isBullish) {
            collateralFundingFee =
                (fundingRate * input.collateral) /
                FUNDING_RATE_PRECISION;
        }

        fee =
            collateralFundingFee +
            (input.collateral * input.leverage * fee) /
            P_1 /
            100; // 1e18 | 1e6 (BaseAsset)
    }
    ...
    function getTradeLiquidationPrice(
        ...
        return
            FundingFeesUtils.getTradeLiquidationPrice(
                input.openPrice,
                input.long,
                input.collateral,
                input.leverage,
    ...
    // Public getters
    function getPairOpenInterestBaseAsset(
        uint256 pairIndex
    ) public view returns (uint256, uint256) {
        return (
            storageT.openInterestBaseAsset(pairIndex, 0),
            storageT.openInterestBaseAsset(pairIndex, 1)
        );
    }
    ...
    function withinMaxGroupOi(
        ...
        return
            (g.maxOi == 0) ||
            ((long ? g.oiLong : g.oiShort) +
                (positionSizeBaseAsset * P_1) /
    ...
    function getCollateralPairMaxOi(
        uint256 pairIndex
    ) external view returns (uint256) {
        return
            (uint256(collateralConfig.precision) * pairOis[pairIndex].max) /
            P_1;
    }
}
