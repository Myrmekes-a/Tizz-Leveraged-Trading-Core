...
import {Base64} from "../external/@openzeppelin/contracts/utils/Base64.sol";
import {Strings} from "../external/@openzeppelin/contracts/utils/Strings.sol";

import "../interfaces/ITizzTokenLockedDepositNftDesign.sol";

/**
 * @custom:version 6.3
 */
contract TizzTokenLockedDepositNftDesign is ITizzTokenLockedDepositNftDesign {
            ...
            Base64.encode(
                bytes.concat(
                    abi.encodePacked(
                        ...
                        Strings.toHexString(
                            uint256(uint160(lockedDeposit.owner)),
                            20
                        ),
                        "</text>"
                    ),
                    ...
                    abi.encodePacked(
                        ...
                        Strings.toString(
                            lockedDeposit.atTimestamp +
                                lockedDeposit.lockDuration
                        ),
                        "</text></svg>"
                    )
                )
