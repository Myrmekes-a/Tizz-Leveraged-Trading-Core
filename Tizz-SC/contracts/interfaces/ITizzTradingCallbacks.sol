// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;

import "./ITizzTradingStorage.sol";

/**
 * @custom:version 7
 */
interface ITizzTradingCallbacks {
    enum TradeType {
        MARKET,
        LIMIT
    }

    enum CancelReason {
        NONE,
        PAUSED,
        MARKET_CLOSED,
        SLIPPAGE,
        TP_REACHED,
        SL_REACHED,
        EXPOSURE_LIMITS,
        PRICE_IMPACT,
        MAX_LEVERAGE,
        NO_TRADE,
        WRONG_TRADE,
        NOT_HIT
    }

    struct AggregatorAnswer {
        uint256 orderId;
        uint256 price;
        uint256 spreadP;
        uint256 open;
        uint256 high;
        uint256 low;
    }

    // Useful to avoid stack too deep errors
    struct Values {
        uint256 posBaseAsset;
        uint256 levPosBaseAsset;
        uint256 tokenPriceBaseAsset;
        int256 profitP;
        uint256 price;
        uint256 liqPrice;
        uint256 baseAssetSentToTrader;
        uint256 reward1;
        uint256 reward2;
        uint256 reward3;
        uint128 collateralPrecisionDelta;
        uint256 collateralPriceUsd;
        bool exactExecution;
    }

    // Internally used struct to avoid stack too deep
    struct RegisterTradeOutput {
        ITizzTradingStorage.Trade finalTrade;
        uint256 tokenPriceBaseAsset; // 1e10
        uint256 collateralPriceUsd; // 1e8
        uint128 collateralPrecisionDelta;
    }

    struct SimplifiedTradeId {
        address trader;
        uint256 pairIndex;
        uint256 index;
        TradeType tradeType;
    }

    struct LastUpdated {
        uint256 tp;
        uint256 sl;
        uint256 limit;
        uint64 created;
    }

    struct TradeData {
        uint256 maxSlippageP; // 1e10 (%)
        uint256 lastOiUpdateTs;
        uint256 collateralPriceUsd; // 1e10 collateral price at trade open
        uint256 _placeholder; // for potential future data
    }

    struct OpenTradePrepInput {
        bool isPaused;
        uint256 executionPrice;
        uint256 wantedPrice;
        uint256 marketPrice;
        uint256 spreadP;
        bool buy;
        uint256 pairIndex;
        uint256 positionSize;
        uint256 leverage;
        uint256 maxSlippageP;
        uint256 tp;
        uint256 sl;
    }

    error WrongParams();
    error Forbidden();

    function openTradeMarketCallback(
        AggregatorAnswer memory a,
        ITizzTradingStorage.PendingMarketOrder memory o
    ) external;

    function closeTradeMarketCallback(
        AggregatorAnswer memory a,
        ITizzTradingStorage.PendingMarketOrder memory o
    ) external;

    function executeNftOpenOrderCallback(
        AggregatorAnswer memory,
        ITizzTradingStorage.PendingNftOrder memory
    ) external;

    function executeNftCloseOrderCallback(
        AggregatorAnswer memory,
        ITizzTradingStorage.PendingNftOrder memory
    ) external;

    function getTradeLastUpdated(
        address,
        uint256,
        uint256,
        TradeType
    ) external view returns (LastUpdated memory);

    function setTradeLastUpdated(
        SimplifiedTradeId calldata,
        LastUpdated memory
    ) external;

    function setTradeData(
        SimplifiedTradeId calldata,
        TradeData memory
    ) external;

    event MarketExecuted(
        uint256 indexed orderId,
        ITizzTradingStorage.Trade t,
        bool open,
        uint256 price,
        uint256 priceImpactP,
        uint256 positionSizeBaseAsset,
        int256 percentProfit, // before fees
        uint256 baseAssetSentToTrader,
        uint256 collateralPriceUsd // 1e8
    );

    event LimitExecuted(
        uint256 indexed orderId,
        uint256 limitIndex,
        ITizzTradingStorage.Trade t,
        address indexed nftHolder,
        ITizzTradingStorage.LimitOrder orderType,
        uint256 price,
        uint256 priceImpactP,
        uint256 positionSizeBaseAsset,
        int256 percentProfit,
        uint256 baseAssetSentToTrader,
        uint256 collateralPriceUsd, // 1e8
        bool exactExecution
    );

    event MarketOpenCanceled(
        uint256 indexed orderId,
        address indexed trader,
        uint256 indexed pairIndex,
        CancelReason cancelReason
    );
    event MarketCloseCanceled(
        uint256 indexed orderId,
        address indexed trader,
        uint256 indexed pairIndex,
        uint256 index,
        CancelReason cancelReason
    );
    event NftOrderCanceled(
        uint256 indexed orderId,
        address indexed nftHolder,
        ITizzTradingStorage.LimitOrder orderType,
        CancelReason cancelReason
    );

    event ClosingFeeSharesPUpdated(
        uint256 baseAssetVaultFeeP,
        uint256 lpFeeP,
        uint256 sssFeeP
    );

    event Pause(bool paused);
    event Done(bool done);
    event GovFeesClaimed(uint256 valueBaseAsset);

    event GovFeeCharged(
        address indexed trader,
        uint256 valueBaseAsset,
        bool distributed
    );
    event OpeningFeeCharged(
        address indexed trader,
        uint256 pairIndex,
        uint256 index,
        uint256 openingFees
    );
    event ClosingFeeCharged(
        address indexed trader,
        uint256 pairIndex,
        uint256 index,
        uint256 closingFees
    );
    event ReferralFeeCharged(address indexed trader, uint256 valueBaseAsset);
    event TriggerFeeCharged(address indexed trader, uint256 valueBaseAsset);
    event SssFeeCharged(address indexed trader, uint256 valueBaseAsset);
    event BaseAssetVaultFeeCharged(
        address indexed trader,
        uint256 valueBaseAsset
    );
    event FundingFeeCharged(
        address indexed trader,
        uint256 pairIndex,
        uint256 tradeValueBaseAsset,
        uint256 feeValueBaseAsset
    );
    event TizzEscrowUpdated(address indexed tizzEscrow);
}
