...
interface ITizzFundingFees {
    ...
    struct Pair {
        ...
        uint48 feeExponent;
        uint256 fundingFees; // collateral precision
        uint256 fundingRate; // 1e4
        bool isBullish;
    }

    struct PairOi {
        uint72 long; // 1e10 (BaseAsset)
        uint72 short; // 1e10 (BaseAsset)
        uint72 max; // 1e10 (BaseAsset)
    ...
    struct FundingFeeInput {
        address trader;
        uint256 pairIndex;
        uint256 index;
        bool long;
        uint256 collateral; // 1e18 | 1e6 (BaseAsset)
        uint256 leverage;
    }
    ...
    function getTradeFundingFee(
        FundingFeeInput memory
    ) external view returns (uint256); // 1e18 | 1e6 (BaseAsset)
